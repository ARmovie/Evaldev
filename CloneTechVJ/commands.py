...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        if cd["update_channel_link"] != None:
            ...
            buttons.append([InlineKeyboardButton('🍿 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 🍿', url=up)])
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAKJ62c6A1cPARdxqV4HjsiHRMMGx8IdAAIjAgACr345VI1cYMt7g0P8NgQ") 
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            files1 = files_
            title = '@armoviechat ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1['file_name'].split()))
